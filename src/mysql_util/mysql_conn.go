...
func init() {
	db, _ = sql.Open("mysql", "username:password@tcp(localhost:3306)/test?charset=utf8")
	// 设置最大连接数
...
func doSomething2() map[string]string {
	dbConn, _ := sql.Open("mysql", "username:password@tcp(localhost:3306)/test?charset=utf8")
	defer dbConn.Close()
